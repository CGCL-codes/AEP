...
inline segment_allocation_state_t * get_sas(void *p)
{
    segment_t* segment = cxl_ptr_segment(p);
    return segment_get_sas(segment);
}

inline segment_t * sas_get_segment(segment_allocation_state_t* sas)
{
...
inline page_t* cxl_segment_page_of(segment_t* segment, void* p)
{
...
inline page_t* cxl_ptr_page(void* p){
    return cxl_segment_page_of(cxl_ptr_segment(p), p);
}

inline void* cxl_page_start(segment_t* segment, page_t* page)
{
    ptrdiff_t idx = page - segment->meta_page;
    return(void*)((char*)segment + (idx*PAGE_SIZE));
}

inline obj_block_t* cxl_page_block_at(void* page_start, uint64_t block_size, uint64_t i)
{
