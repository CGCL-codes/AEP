...
static void *__simple_malloc(size_t n)
{
	...
	if (n > end-cur) {
		...
		if (!cur) {
			brk = __syscall(SYS_pv_brk, 0);
			brk += -brk & PAGE_SIZE-1;
			cur = end = brk;
		}

		if (brk == end && req < SIZE_MAX-brk
		    && __syscall(SYS_pv_brk, brk+req)==brk+req) {
			brk = end += req;
		} else {
			printf("__simple_malloc: use mmap\n");
			...
			void *mem = __mmap(0, req, PROT_READ|PROT_WRITE,
				MAP_PRIVATE|MAP_ANONYMOUS|MAP_USE_PV, -1, 0);
