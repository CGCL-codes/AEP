...
void *pv_aligned_alloc(size_t align, size_t len, int thread_num)
{
	if (thread_num < 0 || thread_num >= NUM_PV_MALLOC_T){ 
		printf("musl lib error: pv_aligned_alloc thread_num incorrect!\n");
		return 0;
	}
	...
	unsigned char *p = pv_malloc(len + align - UNIT, thread_num);
	if (!p)
		return 0;
