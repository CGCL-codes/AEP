...
#else
#undef assert
#define assert(x) do { if (!(x)) {printf("musl lib assert failed\n"); a_crash();} } while(0)
#endif

#define SYS_pv_brk	452
#define brk(p) ((uintptr_t)__syscall(SYS_pv_brk, p))
...
static inline uint64_t get_random_secret()
{
	...
	// DEBUG_PRINT("get_random_secret: libc:0x%lx\n", (unsigned long)&libc);
	// for (size_t i=0; libc.auxv[i]; i+=2){
	// 	DEBUG_PRINT("get_random_secret: auxv[%d]\n", i);
	// 	if (libc.auxv[i]==AT_RANDOM) {
	// 		DEBUG_PRINT("get_random_secret: secret=0x%lx, auxv[i]:0x%lx\n", (unsigned long)secret, (unsigned long)libc.auxv[i]);
	// 		memcpy(&secret, (char *)libc.auxv[i+1]+8, sizeof secret);
	// 	}
	// }
	// DEBUG_PRINT("get_random_secret: secret=0x%lx\n", (unsigned long)secret);
	return secret;
}
