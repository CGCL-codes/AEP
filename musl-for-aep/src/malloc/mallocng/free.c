...
static struct mapinfo pv_nontrivial_free(struct meta *, int, struct malloc_context*);

static struct mapinfo pv_free_group(struct meta *g, struct malloc_context* ctx_local)
{
	struct mapinfo mi = { 0 };
	int sc = g->sizeclass;
	if (sc < 48) {
		ctx_local->usage_by_class[sc] -= g->last_idx+1;
	}
	if (g->maplen) {
		pv_step_seq(ctx_local);
		pv_record_seq(sc, ctx_local);
		mi.base = g->mem;
		mi.len = g->maplen*4096UL;
	} else {
		void *p = g->mem;
		struct meta *m = pv_get_meta_ctx(p, ctx_local);
		int idx = get_slot_index(p);
		g->mem->meta = 0;
		// not checking size/reserved here; it's intentionally invalid
		mi = pv_nontrivial_free(m, idx, ctx_local);
	}
	pv_free_meta(g, ctx_local);
	return mi;
}

static int pv_okay_to_free(struct meta *g, struct malloc_context* ctx_local)
{
	int sc = g->sizeclass;
...
static struct mapinfo pv_nontrivial_free(struct meta *g, int i, struct malloc_context* ctx_local)
{
	uint32_t self = 1u<<i;
	int sc = g->sizeclass;
	uint32_t mask = g->freed_mask | g->avail_mask;

	if (mask+self == (2u<<g->last_idx)-1 && pv_okay_to_free(g, ctx_local)) {
		// any multi-slot group is necessarily on an active list
		// here, but single-slot groups might or might not be.
		if (g->next) {
			assert(sc < 48);
			int activate_new = (ctx_local->active[sc]==g);
			dequeue(&ctx_local->active[sc], g);
			if (activate_new && ctx_local->active[sc])
				activate_group(ctx_local->active[sc]);
		}
		return pv_free_group(g, ctx_local);
	} else if (!mask) {
		assert(sc < 48);
		// might still be active if there were no allocations
		// after last available slot was taken.
		if (ctx_local->active[sc] != g) {
...
//must use it to free p in the current context(thread)
void pv_free(void *p, int thread_num)
{
	if (!p) return;

	if (thread_num < 0 || thread_num >= NUM_PV_MALLOC_T) return;

	//printf("enter pv_free: p=%lx\n", (unsigned long)p);
	...
	*(uint16_t *)((char *)p-2) = 0;
	...
	struct mapinfo mi = pv_nontrivial_free(g, idx, ctx_local);
	//unlock();
	if (mi.len) {
		int e = errno;
		//munmap(mi.base, mi.len);
		pv_pool_free_pages(mi.base, calculate_page_count(mi.len));
		errno = e;
	}
}
