...
pid_t kernel_clone(struct kernel_clone_args *args)
{
	...
	if (current->aep){
		aep_child_small_init(p, current->aep);
	}
#endif
...
pid_t aep_kernel_clone(struct kernel_clone_args *args, int template_id)
{
	u64 clone_flags = args->flags;
	struct completion vfork;
	struct pid *pid;
	...
	if (!(clone_flags & CLONE_VM)){
		//aep init using template
		aep_child_init(p, mutant_templates[template_id]);
	}
...
#else /* CONFIG_ATOMIC_EXECUTION_PROTECTION */
