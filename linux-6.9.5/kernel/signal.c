...
bool get_signal(struct ksignal *ksig)
{
	...
	struct signal_struct *signal = current->signal;
	int signr;
	struct pt_regs *aep_regs = task_pt_regs(current);
...
relock:
	...
	fatal:
		...
		if (current->aep && ADDR_IN_PV_REGION(aep_regs->ip) && (!ADDR_IN_SHADOW_ENTRY(aep_regs->ip))){
			//printk("aep sig handler, address: %lx, delay!\n", aep_regs->ip);
			aep_unmap_shadow_entry(current->aep, current->aep->cb.entry);
			return false;
		}

		if (current->aep && ADDR_IN_SHADOW_ENTRY(aep_regs->ip)){
			printk("aep sig handler reach shadow entry, true kill!\n");
		}
