			      ...
			      unsigned long address)
{
	...
	if (current->aep && ADDR_IN_PV_REGION(address)){
		printk("aep prcess fault in aep space, addr: %lx\n", address);
		if (ADDR_IN_SHADOW_ENTRY(address)){
			printk("aep get shadow entry pagefault: %d\n", current->pid);
		...
		printk("aep todo: other pv region page fault handle\n");
	}
#endif
