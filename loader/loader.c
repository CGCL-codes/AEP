...
static unsigned char *prepare_buffer(int fd, ssize_t size){
	return z_mmap(NULL, size, PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
}

static unsigned long loadelf_anon_in_pv(int fd, Elf_Ehdr *ehdr, Elf_Phdr *phdr)
{
	unsigned long minva, maxva;
	Elf_Phdr *iter;
	ssize_t sz;
	...
	for (iter = phdr; iter < &phdr[ehdr->e_phnum]; iter++) {
		...
		z_printf("start: %lx, sz: %lx, p_memsz: %lx\n", start, sz, iter->p_memsz);

		z_printf("p_type: %lx, p_flags: %lx, p_offset: %lx, p_vaddr: %lx, p_paddr: %lx, p_filesz: %lx, p_memsz: %lx, p_align, %lx\n", 
										iter->p_type,
										iter->p_flags,
										iter->p_offset,
										iter->p_vaddr,
		...
		z_printf("point-1, iter->p_offset=0x%lx\n", iter->p_offset);
		if (z_lseek(fd, iter->p_offset, SEEK_SET) < 0)
			goto err;
		z_printf("point-2, fd = %d, p + off = 0x%lx, iter->p_filesz=%lx\n", fd, (unsigned long)(p + off), 
																	(unsigned long)iter->p_filesz);
		char *transbuf = alloca(iter->p_filesz);
		if (z_read(fd, transbuf, iter->p_filesz) != (ssize_t)iter->p_filesz)
			goto err;
		
		z_memcpy(p + off, transbuf, iter->p_filesz);
		//if (z_read(fd, p + off, iter->p_filesz) !=
		//		(ssize_t)iter->p_filesz)
		//	goto err;
		// z_printf("point-3\n");
...
void exec_pv_elf(const char *file, int argc, char *argv[])
{
	Elf_Ehdr ehdrs[2], *ehdr = ehdrs;
	Elf_Phdr *phdr, *iter;
	...
	for (i = 0;; i++, ehdr++) {
		...
		if ((fd = z_open(file, O_RDONLY)) < 0)
			z_errx(1, "can't open %s", file);
		z_printf("open exec success\n");
		if (z_read(fd, ehdr, sizeof(*ehdr)) != sizeof(*ehdr))
			z_errx(1, "can't read ELF header %s", file);
		z_printf("read exec success\n");
		if (!check_ehdr(ehdr))
			z_errx(1, "bogus ELF header %s", file);
		...
		if (z_read(fd, phdr, sz) != sz)
			z_errx(1, "can't read program header %s", file);
		/* Time to load ELF. */
		if ((base[i] = loadelf_anon_in_pv(fd, ehdr, phdr)) == LOAD_ERR)
			z_errx(1, "can't load ELF %s", file);
	...
	enter_main = (int (*)(int,  char **))entry[Z_PROG];
	z_printf("entry: %lx\n", (unsigned long)enter_main);

	enter_main(argc, argv);
}